...
class BlockToken(val defenseValue: Int, val properties: List<DefenseProperty> = listOf()): CombatToken() {
    enum class DefenseProperty {
        FIRE, ICE
    }
}

class AttackToken(val attackValue: Int, val properties: List<AttackProperty> = listOf()): CombatToken() {
