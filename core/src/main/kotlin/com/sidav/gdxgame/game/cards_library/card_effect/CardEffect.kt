...
open class CardEffect(
    ...
    protected var paidWith: ManaUnit? = null
    open fun resetState() {
        paidWith = null
    ...
    fun currentRequest(): CardEffectRequest {
        val req = when (state) {
            ...
            States.PAYMENT_ACCEPTED -> ConsumePayment(
                paidWith ?: error("Consume payment requested, but no mana paid")
            )
        ...
        debugMsg(
            "CARD_EFFECT",
            "Current effect in state ${state} is requesting ${req::class.simpleName}"
        )
        return req
    }
    ...
    fun onEvent(event: CardEffectEvent) {
        debugMsg(
            "CARD_EFFECT",
            "Effect ${this::class.simpleName ?: "<AnonymousClass>"} (state $state) received an event: ${event::class.simpleName}"
        )
        // Thread.sleep(1000)
        when (state) {
            States.INITIAL_STATE -> {
                if (event is CardEffectEvent.NoEvent) {
                    if (cost is ManaCost.Free) switchState(States.APPLYING)
                    else switchState(States.WAITING_FOR_PAYMENT)
                }
            }
    ...
    open fun onWaitingForPayment(ev: CardEffectEvent) {
        if (ev is CardEffectEvent.ManaPaid && cost.acceptsAsPayment(ev.manaUnit)) {
            paidWith = ev.manaUnit
