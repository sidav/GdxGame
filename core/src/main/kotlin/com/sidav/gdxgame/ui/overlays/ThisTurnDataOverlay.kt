...
/** Shows current ambient mana, movement, influence etc */
class ThisTurnDataOverlay(val gameState: GameState) : OverlayBase() {
    ...
    override fun render() {
        ...
        // Dice rects
        for ((i, dice) in gameState.manaStock.ambientManaDice.withIndex()) {
            shape.color = getColorForMana(dice.color)
            shape.rect(
...
//        shape.end()
        ...
        batch.begin()
        font.draw(batch,
            "MOV ${gameState.player.stats.movement} | INF ${gameState.player.stats.influence}",
            100f, screenHeight - 5)
        batch.end()
    }
}
